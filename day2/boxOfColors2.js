function colors(input) {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        //let isPossible=true;
        let gameArr=arr[i].split(':');
        let gameId=Number(gameArr.shift().split(" ")[1]);
        let sets=gameArr[0].split(';')
        gameObj.game=gameId;

        for (let j = 0; j < sets.length; j++) {
            let gameColors=sets[j].trim().split(',');

            for (let k = 0; k < gameColors.length; k++) {
                let gameColorsArr=gameColors[k].trim().split(' ');
                if (gameObj[gameColorsArr[1]]<=Number(gameColorsArr[0])) {
                    gameObj[gameColorsArr[1]]=Number(gameColorsArr[0]);
                }
                
            }
        }

        let multiplied=gameObj.red*gameObj.blue*gameObj.green;
...
colors(`Game 1: 13 red, 18 green; 5 green, 3 red, 5 blue; 5 green, 9 red, 6 blue; 3 blue, 3 green
